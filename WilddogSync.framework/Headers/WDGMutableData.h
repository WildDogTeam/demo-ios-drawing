...
/**
  WDGMutableData 实例是 Wilddog 节点处的另一种数据载体，当使用 runTransactionBlock：方法时，
  你会接受到一个包含当前节点数据的 WDGMutableData 实例。如果你想要保存此节点的数据，将此节点的
  WDGMutableData 传参到 [FTransactionResult successWithValue:] 方法中。
 ...
 */
@interface WDGMutableData : NSObject
 ...
 * @return 如果在指定的相对路径下，该 WDGMutableData 包含子节点，则返回YES
 */
 ...
 *
 * @param path 可以是类似'child'的单层级路径，也可以是类似'a/deeper/child'多层级路径
 * @return 指定路径下的 WDGMutableData 实例
 */
