...
/**
 * queryLimitedToFirst: 用于创建一个新WDGSyncQuery引用，获取从第一条开始的指定数量的数据。
 * 返回的WDGSyncQuery查询器类将响应从第一个开始，到最多指定(limit)节点个数的数据。
 *
 * @param limit 这次查询能够获取的子节点的最大数量
 * @return 返回一个WDGSyncQuery查询器类，最多指定(limit)个数的数据
 */
- (WDGSyncQuery *)queryLimitedToFirst:(NSUInteger)limit;


/**
 * queryLimitedToLast: 用于创建一个新WDGSyncQuery引用，获取从最后一条开始向前指定数量的数据。
...
/**
 * queryOrderedByChild: 用于产生一个新WDGSyncQuery引用，是按照特定子节点的值进行排序的。
 * 此方法要与 queryStartingAtValue:, queryEndingAtValue: 或 queryEqualToValue: 方法联合使用。
 *
 * @param key 指定用来排序的子节点的key
 * @return 返回一个按指定的子节点key排序生成的WDGSyncQuery查询器类
 */
- (WDGSyncQuery *)queryOrderedByChild:(NSString *)key;


/**
 * queryOrderedByKey 用于产生一个新WDGSyncQuery引用，是按照特定子节点的key进行排序的。
 * 此方法要与 queryStartingAtValue:, queryEndingAtValue: 或 queryEqualToValue: 方法联合使用。
 *
 * @return 返回一个按指定的子节点key排序生成的WDGSyncQuery查询器类
 */
...
/**
 * queryOrderedByValue 用于产生一个新WDGSyncQuery引用，是按照当前节点的值进行排序的。
 * 此方法要与 queryStartingAtValue:, queryEndingAtValue: 或 queryEqualToValue: 方法联合使用。
 *
 * @return 返回一个按指定的子节点值排序生成的WDGSyncQuery查询器类
 */
- (WDGSyncQuery *)queryOrderedByValue;


/**
 * queryOrderedByPriority 用于产生一个新WDGSyncQuery引用，是按照当前节点的优先级排序的。 
 * 此方法要与 queryStartingAtValue:, queryEndingAtValue: 或 queryEqualToValue: 方法联合使用。
 *
 * @return 返回一个按指定的子节点优先级排序生成的WDGSyncQuery查询器类
 */
- (WDGSyncQuery *)queryOrderedByPriority;


/**
 * queryStartingAtValue: 用于返回一个WDGSyncQuery引用，这个引用用来监测数据的变化，这些被监测的数据的值均大于或等于startValue。
 *
 * @param startValue query查询到的值均大于等于startValue
 * @return 返回一个WDGSyncQuery查询器类，用于响应在数据值大于或等于startValue的节点事件
 */
- (WDGSyncQuery *)queryStartingAtValue:(nullable id)startValue;


/**
 * queryStartingAtValue:childKey: 用于返回一个WDGSyncQuery引用，这个引用用来监测数据的变化，这些被监测的数据的值大于startValue，或者等于startValue并且key大于等于childKey。
 *
 * @param startValue query查询到的值均大于等于startValue
 * @param childKey  当 query 查询到的值和 startValue 相等时，则比较 key 的大小。
 * @return 返回一个WDGSyncQuery查询器类，用于响应在数据值大于startValue，或等于startValue的值并且key大于或等于childKey的节点事件
 */
- (WDGSyncQuery *)queryStartingAtValue:(nullable id)startValue childKey:(nullable NSString *)childKey;


/**
 * queryEndingAtValue: 用于返回一个WDGSyncQuery引用，这个引用用来监测数据的变化，这些被监测的数据的值均小于或者等于endValue。
 *
 * @param endValue query查询到的值均小于等于endValue
 * @return 返回一个WDGSyncQuery查询器类，用于响应在数据值均小于或等于endValue的节点事件
 */
- (WDGSyncQuery *)queryEndingAtValue:(nullable id)endValue;


/**
 * queryEndingAtValue:childKey: 用于返回一个WDGSyncQuery引用，这个引用用来监测数据的变化，这些被监测的数据的值小于endValue，或者等于endValue并且key小于等于childKey。
 *
 * @param endValue query查询到的值均小于等于endValue
 * @param childKey 当 query 查询到的值和 endValue 相等时，则比较它们 key 的大小。
 * @return 返回一个WDGSyncQuery查询器类，用于响应在查询到的数据值小于endValue，或者数据值等于endValue并且key小于等于childKey的节点事件
 */
- (WDGSyncQuery *)queryEndingAtValue:(nullable id)endValue childKey:(nullable NSString *)childKey;


/**
 * queryEqualToValue: 用于返回一个WDGSyncQuery引用，这个引用用来监测数据的变化，这些被监测的数据的值都等于value。
 *
 * @param value query查询到的值都等于value
 * @return 返回一个WDGSyncQuery查询器类，用于响应这个与之相等数值节点事件
 */
- (WDGSyncQuery *)queryEqualToValue:(nullable id)value;


/**
 * queryEqualToValue:childKey: 用于返回一个WDGSyncQuery引用，这个引用用来监测数据的变化，这些被监测的数据的值等于value并且key等于childKey。返回的值肯定是唯一的，因为key是唯一的。
 *
 * @param value query查询到的值都等于value
 * @param childKey query查询到的key都等于childKey
 * @return 返回一个WDGSyncQuery查询器类，用于响应这个与之相等数值和key节点事件
 */
