...
static NSString * const kWildebaseURL = @"https://draw.wilddogio.com";
...
- (id)init
{
    self = [super init];
    if (self != nil) {
        // initialize the WildDog that is used for this sample
        self.wilddog = [[Wilddog alloc] initWithUrl:kWildebaseURL];
        ...
        self.childAddedHandle = [self.wilddog observeEventType:WEventTypeChildAdded withBlock:^(WDataSnapshot *snapshot) {
...
- (void)dealloc
{
    ...
    [self.wilddog removeObserverWithHandle:self.childAddedHandle];
}
...
- (void)drawView:(WDDrawView *)view didFinishDrawingPath:(WDPath *)path
{
    ...
    Wilddog *pathRef = [self.wilddog childByAutoId];
    ...
    // save the path to WildDog
    [pathRef setValue:[path serialize] withCompletionBlock:^(NSError *error, Wilddog *ref) {
